...
flowchart LR
  ...
  UI -->|text_or_speech| API
  API -->|normalize_slots| NLU
  API -->|evaluate_tree| TREE
  TREE -->|next_question_and_rationale| API
  API -->|response| UI
  API -->|session_context| STATE
```
